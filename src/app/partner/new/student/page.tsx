...
import { StateSchema, StudentCreateRequestPayload } from "@/app/api/schema";
...
export default function EmployeePage() {
  ...
  const mutation = trpc.createStudent.useMutation({
    onSuccess: () => { route.back() }
  })

  const form = useForm<StudentCreateRequestPayload>({
    resolver: zodResolver(StudentCreateRequestPayload),
    defaultValues: {
      username: "",
      password: "",
  ...
  const onSubmit: SubmitHandler<StudentCreateRequestPayload> = (data) => {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6 p-6">
        <div>
          <h3 className="text-lg font-medium">Cadastro de Estudante</h3>
        </div>
        <Form {...form}>
